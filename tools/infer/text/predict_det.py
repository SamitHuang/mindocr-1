...
class TextDetector(object):
    def __init__(self, args):
        ...
        if ckpt_dir is None:
            ...
            pretrained = False
        assert args.det_algorithm in algo_to_model_name, f'Invalid det_algorithm {args.det_algorithm}. Supported detection algorithms are {list(algo_to_model_name.keys())}'
        ...
        print('INFO: Init detection model: {} --> {}. Model weights loaded from {}'.format(args.det_algorithm, model_name, 'pretrained url' if pretrained else ckpt_load_path))

        # build preprocess and postprocess
        self.preprocess = Preprocessor(task='det',
    ...
    def __call__(self, img_or_path, do_visualize=True):
        ...
        if do_visualize:
            ...
            show_imgs([data['image']], title=fn+'_det_preprocessed',
                      ...
                      show=False, save_path=os.path.join(self.vis_dir, fn+'_det_preproc.png'))
...
def save_det_res(det_res_all: List[dict], img_paths: List[str], include_score=False, save_path='./det_results.txt'):
    ...
    for i, det_res in enumerate(det_res_all):
        if not include_score:
            img_pred = os.path.basename(img_paths[i]) + "\t" + str(json.dumps([x.tolist() for x in det_res['polys']])) + "\n"
        else:
            img_pred = os.path.basename(img_paths[i]) + "\t" + str(json.dumps([x.tolist() for x in det_res['polys']])) + '\t' + str(json.dumps(det_res['scores'].tolist())) + "\n"
